...
test-gui:
	...
	@if $(PYTHON) -c "import playwright" 2>/dev/null; then \
		$(PYTHON) -m pytest $(TESTS_DIR)/python/test_gui.py $(TESTS_DIR)/python/test_file_modification.py -v --tb=short --browser chromium --browser firefox --browser webkit -n 8 --reruns 2 --reruns-delay 1; \
	else \
