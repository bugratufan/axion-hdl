...
| GUI-EDIT-028 | Above Register Conflict | When user sets address conflicting with register above, warning shown but no automatic shifting. | Playwright (`gui.test_edit_028`) |
| GUI-EDIT-029 | Address Revert | User can revert address to original using revert button. | Playwright (`gui.test_edit_029`) |
...
| GUI-EDIT-032 | Real-time Update | All address changes appear immediately without page reload. | Playwright (`gui.test_edit_032`) |
...
| GUI-EDIT-038 | Address Persistence | When saving, auto-assigned addresses are persisted to the output file (VHDL, JSON, YAML, XML). | Playwright (`gui.test_edit_038`) |
