...
| `--hier FILE` | Hierarchy file for centralized base address assignment (YAML, TOML, JSON, or XML). Overrides `base_addr` in all individual module files. When the same module appears multiple times, the `instance` field names the output files. Also generates `address_map.html` plus combined `address_map.h`, `address_map_pkg.vhd`, and `address_map_pkg.sv` base-address maps. |
