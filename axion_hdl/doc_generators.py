...
class AddressMapExportGenerator:
    """Generator for machine-includable combined base-address maps.

    Produces ``address_map.h`` (C), ``address_map_pkg.vhd`` (VHDL package) and
    ``address_map_pkg.sv`` (SystemVerilog package). Each file defines a single
    ``<PREFIX>_BASE_ADDR`` constant per module instance, so firmware and HDL can
    reference every block's base address from one source of truth. Per-register
    offsets remain in the per-module ``<module>_regs.h`` headers.

    Unlike :class:`AddressMapHTMLGenerator`, this generator does **not** filter out
    canonical (``_hide_from_docs``) entries: the combined address-map files list
    every generated register space, while the HTML report shows only the deployed
    named instances.
    """
    ...
    def generate(self, modules: List[Dict]) -> List[str]:
        """Generate all three address-map export files.

        Args:
            modules: Analyzed (and hierarchy-applied) module dicts. Canonical
                entries are intentionally included.
        ...
        outputs = []
        outputs.append(self._write('address_map.h', self._render_c(blocks)))
        outputs.append(self._write('address_map_pkg.vhd', self._render_vhdl(blocks)))
        outputs.append(self._write('address_map_pkg.sv', self._render_sv(blocks)))
        return outputs
    ...
    def _c_ident(name: str) -> str:
        """Sanitize a name into an UPPERCASE C/VHDL/SV-safe identifier."""
        ident = re.sub(r'[^A-Za-z0-9_]', '_', name)
        ident = re.sub(r'_+', '_', ident).strip('_') or 'module'
        if ident[0].isdigit():
            ident = 'v_' + ident
        return ident.upper()

    def _collect_blocks(self, modules: List[Dict]) -> List[Dict]:
        """Build a sorted list of address-map blocks (one base address each).

        Each block: {'instance', 'module', 'prefix', 'base'}.
        """
        blocks = []
        for module in modules:
            display_name = module.get('_effective_name', module['name'])
            blocks.append({
                'instance': display_name,
                'module': module['name'],
                'prefix': self._c_ident(display_name),
                'base': self._to_int(module.get('base_address', 0)),
            })
    ...
    @staticmethod
    def _col_width(blocks) -> int:
        return max((len(f"{b['prefix']}_BASE_ADDR") for b in blocks), default=1)

    def _render_c(self, blocks: List[Dict]) -> str:
        w = self._col_width(blocks)
        lines = [
            "/**",
            " * @file address_map.h",
            " * @brief Combined base-address map for all module instances",
            " * @note Generated by Axion HDL - Do not edit manually",
            " */",
            "",
        ...
        for b in blocks:
            lines.append(
                f"#define {b['prefix'] + '_BASE_ADDR':<{w}}    0x{b['base']:08X}U"
                f"  /* module: {b['module']} */"
            )
        lines.append("")
        lines.append("#endif /* ADDRESS_MAP_H */")
        return '\n'.join(lines) + '\n'

    def _render_vhdl(self, blocks: List[Dict]) -> str:
        w = self._col_width(blocks)
        lines = [
            "-- Combined base-address map for all module instances",
            "-- Generated by Axion HDL - Do not edit manually",
            "",
            "library ieee;",
        ...
        for b in blocks:
            lines.append(
                f"    constant {b['prefix'] + '_BASE_ADDR':<{w}} : "
                f"std_logic_vector(31 downto 0) := x\"{b['base']:08X}\";"
                f"  -- module: {b['module']}"
            )
        lines.append("end package address_map_pkg;")
        return '\n'.join(lines) + '\n'

    def _render_sv(self, blocks: List[Dict]) -> str:
        w = self._col_width(blocks)
        lines = [
            "// Combined base-address map for all module instances",
            "// Generated by Axion HDL - Do not edit manually",
            "",
            "`ifndef ADDRESS_MAP_PKG_SV",
            "`define ADDRESS_MAP_PKG_SV",
            "",
            "package address_map_pkg;",
        ]
        for b in blocks:
            lines.append(
                f"    localparam logic [31:0] {b['prefix'] + '_BASE_ADDR':<{w}} = "
                f"32'h{b['base']:08X};  // module: {b['module']}"
            )
        lines.append("endpackage : address_map_pkg")
        lines.append("")
        lines.append("`endif  // ADDRESS_MAP_PKG_SV")
        return '\n'.join(lines) + '\n'
