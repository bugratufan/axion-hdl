...
class SourceModifier:
    ...
    def _generate_axion_tag(self, reg: Dict, existing_tag_content: str = None) -> str:
        ...
        # 2. Addr - Always include address (persist auto-assigned addresses)
        # Previously only included when manual_address was True
        addr_val = reg.get('address')
        
        if addr_val:
            # Always include address to persist auto-assigned addresses
            ...
            if isinstance(addr_val, str) and not addr_val.lower().startswith('0x'):
             ...
             # Fallback: Preserve ADDR if it was already there
